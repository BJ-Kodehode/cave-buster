  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        router.push(`/movies/${result.data._id}`);
        router.refresh();
      } else {
        const error = await response.json();
        alert(error.message || "Kunne ikke opprette filmen");
      }
    } catch (error) {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="mb-6">
        ...
        <h1 className="text-3xl font-bold text-white">Legg til ny film</h1>
        <p className="text-gray-400 mt-2">Fyll ut informasjon om filmen</p>
      </div>
